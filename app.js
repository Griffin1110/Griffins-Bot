...
var registerCommand = function(command) {
    ...
    // REGISTER COMMANDS BELOWcommands\
...
//registerCommand(require('./commands/mod.js').ban); // When you feel like it's necessary xD
...
client.on('ready', () => {
    ...
    music(client, {
        ...
        channel: 'Music Chat' // Name of voice channel to join. If omitted, will instead join user's voice channel.
    });
});
...
client.on("message", message => {
    ...
    if (msgcontent.startsWith('g.save') || msgcontent.startsWith('g.bot') || msgcontent.startsWith('g.eval')) {
        ...
        let pushEmbed = function(finished) {
            ...
            delete embed; // Conserving memory :P
            didOnce = true;
            commandString = "";
            descString = "";
        }
...
saveConfig(false); // Saves config every hour and 30 mins. Do g.dev save to save earlier

stdin.addListener("data", function(d) { // Console magic! Type stuff in the console and watch stuff happen!
    let input = d.toString().trim();
    if (input.toLowerCase().startsWith("eval")) { // Starting with eval makes stuff eval xD (Example: eval client.guilds)
        try {
